...
public class UnitSum {
    public static class PassMapper extends Mapper<Object, Text, Text, DoubleWritable> {
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            ...
            String toPage = line.split("\t")[0];
            Double probValue = Double.parseDouble(line.split("\t")[1]);
            context.write(new Text(toPage), new DoubleWritable(probValue));
        }
    }
    ...
    public static class SumReducer extends Reducer<Text, DoubleWritable, Text, DoubleWritable> {
        ...
        public void reduce(Text key, Iterable<DoubleWritable> values, Context context)
           ...
           //input key = toPage value = <unitMultiplication>
            //target: sum!
            double sum = 0;
            for (DoubleWritable prValue : values) {
                sum += prValue.get();
            }
